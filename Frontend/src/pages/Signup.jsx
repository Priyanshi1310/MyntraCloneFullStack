...
import { useNavigate, Link } from "react-router-dom";
import { useDispatch } from "react-redux";
import { signupUser, loginUser } from "../redux/user/userSlice";
import { FaGoogle } from "react-icons/fa";
...
const Signup = () => {
  ...
  const handleEmailPasswordSignup = async (event) => {
    ...
    try {
      ...
      if (error?.error?.includes("User already exists")) {
        ...
        setSignupError(error?.error || "Signup failed. Please try again.");
      }
      setLoading(false);
    }
  };
  ...
  return (
    <div className="m-auto">
      <div className="flex flex-col items-center">
        <img className="sm:w-2/4 w-full h-52 object-cover lg:w-1/4 mx-auto" src={poster} alt="poster" />
        <div className="lg:w-1/4 sm:w-2/4 p-3 flex flex-col items-center">
          <h1 className="text-xl font-bold my-6">Signup</h1>
          <form className="flex flex-col gap-6">
            <input type="name" placeholder="Enter Name" name="name" className="border-2 rounded p-2 text-lg outline-none text-gray-700" onChange={handleFormDataChange} />
            <input type="email" placeholder="Enter email" name="email" className="border-2 rounded p-2 text-lg outline-none text-gray-700" onChange={handleFormDataChange} />
            <input type="password" placeholder="Enter password" name="password" className="border-2 rounded p-2 text-lg outline-none text-gray-700" onChange={handleFormDataChange} />
            {signupValidation && <p className="text-sm text-red-600">{signupValidation}</p>}
            <p className="text-sm text-gray-500">By continuing, I agree to the <span className="text-pink-600 font-bold">Terms of Use</span> & <span className="text-pink-600 font-bold">Privacy Policy</span></p>
            <button className="p-3 font-bold bg-pink-600 text-white" onClick={handleEmailPasswordSignup}>{loading ? "Loading..." : "Continue"}</button>
            {signupError && <p className="p-2 text-sm text-red-600">{signupError}</p>}
            <p className="text-sm text-gray-500">Already a member? <Link to="/signin" className="text-pink-600 font-semibold cursor-pointer">Sign in</Link></p>
          </form>
        </div>
      </div>
    </div>

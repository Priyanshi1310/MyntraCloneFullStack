...
const Signin = () => {
  ...
  const handleEmailPasswordSignIn = async (event) => {
    ...
    try {
      ...
      setSigninError("Wrong credentials, Try again");
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="lg:w-1/4 sm:w-2/4 p-3 flex flex-col items-center">
          ...
          <form className="flex flex-col gap-6">
            ...
            <button className="p-3 font-bold bg-pink-600 text-white" onClick={handleEmailPasswordSignIn}>{loading ? "Loading..." : "Continue"}</button>
            {signinError && <p className="p-2 text-sm text-red-600">{signinError}</p>}
            <p className="text-sm text-gray-500">Not a member yet? <Link to="/signup" className="text-pink-600 font-semibold cursor-pointer">Sign up</Link></p>
          </form>
        </div>
      </div>
    </div>
